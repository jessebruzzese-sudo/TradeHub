...
import { Calendar, Clock, DollarSign, MapPin, ArrowLeft, CheckCircle, XCircle, Ban, AlertCircle, Flag, MessageSquare, Edit } from 'lucide-react';
...
import { canAdminApplyToJob } from '@/lib/admin-utils';
...
export default function JobDetailPage() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (jobData) {
          ...
          if (jobData.contractor_id) {
            const existingContractor = store.getUserById(jobData.contractor_id);
    ...
    fetchJobIfNeeded();
  }, [jobId, store, isLoadingJob]);

  const job = store.getJobById(jobId);
  const contractor = job ? store.getUserById(job.contractorId) : null;
  ...
  const isSubcontractor = !isContractor && currentUser?.role === 'subcontractor';
  const isAdmin = currentUser?.role === 'admin';

  const lifecycleState = job ? getJobLifecycleState(job, applications.length > 0) : null;
  const canApply = isSubcontractor && lifecycleState?.allowsApplications && !myApplication;
  const canWithdraw = myApplication ? canWithdrawApplication(myApplication.status, job?.status || 'open') : false;
  const canMessage = isSubcontractor && !isMyJob && job?.status !== 'cancelled' && job?.status !== 'closed';
  ...
  const canCancelJob = lifecycleState?.canCancel && (isMyJob || job.selectedSubcontractor === currentUser.id || job.confirmedSubcontractor === currentUser.id);
  const canLeaveReview = job.status === 'cancelled' && canLeaveReliabilityReview(job, currentUser.id);
  const existingReview = canLeaveReview ? store.getReviewsByJob(job.id).find((r) => r.authorId === currentUser.id) : null;

  const recipientId = isMyJob
    ? job.confirmedSubcontractor || job.selectedSubcontractor
    : job.contractorId;
  const recipient = recipientId ? store.getUserById(recipientId) : null;
  const dashboardHref = isContractor ? '/dashboard/contractor' : '/dashboard/subcontractor';
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 mb-6">
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-6">
            <div className="flex items-center gap-2 text-gray-700">
              ...
              <span>{job.location}, {job.postcode}</span>
            </div>
            <div className="flex items-center gap-2 text-gray-700">
          ...
          <div className="mb-6">
            ...
            <Link href={`/users/${contractor.id}`}>
              <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer border border-transparent hover:border-gray-200">
                <UserAvatar avatarUrl={contractor.avatar} userName={contractor.name} size="md" />
                <div>
                  <p className="font-medium text-gray-900 hover:text-blue-600 transition-colors">{contractor.name}</p>
                  <p className="text-sm text-gray-600">{contractor.businessName}</p>
                </div>
              </div>
            </Link>
          </div>
          ...
          {myApplication && (
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                ...
                {canMessage && (
                  <Button onClick={handleMessagePoster} variant="outline" size="sm" className={canWithdraw ? "flex-1" : "w-full"}>
                    <MessageSquare className="w-4 h-4 mr-2" />
                    Message Poster
                  </Button>
                )}
          ...
          {!isContractor && job.status === 'accepted' && job.selectedSubcontractor === currentUser.id && (
            <div className="flex gap-3">
          ...
          {isMyJob && job.status === 'accepted' && (
            <div className="flex gap-3">
              <Button
                ...
                className="flex-1"
                disabled={!lifecycleState?.canConfirmHire}
              >
                <CheckCircle className="w-4 h-4 mr-2" />
                Confirm Hire
              </Button>
          ...
          {job.status === 'cancelled' && job.cancelledAt && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              {job.cancellationReason && (
                <p className="text-sm text-gray-700 mt-2">{job.cancellationReason}</p>
              )}
            </div>
          )}
        </div>
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      {job.status === 'open' && lifecycleState?.allowsSelection && (
                        <Button
                          ...
                          disabled={lifecycleState?.isExpired}
                        >
                          Select
                        </Button>
                      )}
        ...
        <Dialog open={showApplyDialog} onOpenChange={setShowApplyDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Tell the contractor why you're a great fit for this job
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <Textarea
        ...
        <Dialog open={showWithdrawDialog} onOpenChange={setShowWithdrawDialog}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <Textarea
                placeholder="Optional: Reason for withdrawing (visible to contractor)"
        ...
        <Dialog open={showCloseDialog} onOpenChange={setShowCloseDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                This will close the job without hiring anyone. You can reopen it later if needed.
              </DialogDescription>
            </DialogHeader>
            <div className="flex gap-3">
