...
export const PUT = async (request:any, context:any) => {
	...
	try{
		await jobRepo.declineApplication(job, app);	
	}catch(err_){
		console.error(err_);
		return NextResponse.json({ msg: "Could not decline application" }, { status: 500 });
	}
	return NextResponse.json({ msg: "Job declined" }, { status: 200 });
};
