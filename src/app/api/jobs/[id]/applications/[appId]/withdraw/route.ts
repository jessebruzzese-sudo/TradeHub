...
// withdraw application for job
// status should declined
// update updatedAt timestamp and set reason
// remove application from selected table
export const PUT = async (request:any, context:any) => {
	...
	let payload = null;
	try{
		payload = WithdrawApplicationSchema.parse(await request.json());
	}catch(err_){
		return NextResponse.json({ msg: "Failed to parse payload" }, { status: 400 });
	}
	...
	try{
		await jobRepo.withdrawApplication(job, app, payload.reason);	
	}catch(err_){
		console.error(err_);
		return NextResponse.json({ msg: "Could not withdraw application" }, { status: 500 });
	}
	return NextResponse.json({ msg: "Application withdrawn" }, { status: 200 });
};
