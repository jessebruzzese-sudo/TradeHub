  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-slate-200/80 bg-gradient-to-b from-slate-50 via-slate-100 to-slate-200 p-4 shadow-sm sm:p-6">
      ...
      <div className="relative z-10 space-y-3 sm:space-y-5">
        <button
          type="button"
          onClick={() => setIsProfileStrengthOpen((current) => !current)}
          aria-expanded={isProfileStrengthOpen}
          aria-controls="profile-strength-content"
          className="group w-full rounded-2xl border border-slate-200/90 bg-white/90 p-4 text-left shadow-sm backdrop-blur-sm transition hover:bg-white md:p-5"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto flex items-center gap-2 sm:gap-3">
              ...
              <ChevronDown
                className={`h-4 w-4 text-slate-500 transition-transform duration-300 sm:h-5 sm:w-5 ${isProfileStrengthOpen ? 'rotate-180' : 'rotate-0'}`}
                aria-hidden
              />
            </div>
          </div>
          {!isProfileStrengthOpen ? (
            <p className="mt-1.5 text-xs text-slate-500">See score breakdown and improvement tips</p>
          ) : null}
        </button>

        <div
          id="profile-strength-content"
          aria-hidden={!isProfileStrengthOpen}
          className={`overflow-hidden transition-all duration-300 ease-out ${
            isProfileStrengthOpen ? 'visible max-h-[2600px] opacity-100' : 'invisible max-h-0 opacity-0'
          }`}
        >
